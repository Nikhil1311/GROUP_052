...
class Agent(BaseAgent):
    ...
    def __init__(self, env_specs, obs_dim=11, action_dim=3, hidden_dim=1024, hidden_depth=2, action_range=[-1, 1], device=device,
                 discount=0.99, init_temperature=0.2, alpha_lr=3e-4, alpha_betas=[0.9, 0.999],
                 actor_lr=1e-3, actor_betas=[0.9, 0.999], actor_update_frequency=200, critic_lr=1e-3,
                 critic_betas=[0.9, 0.999], critic_tau=0.005, critic_target_update_frequency=2,
                 batch_size=256, learnable_temperature=False, kl_weight=100000):
        ...
        self.actor_frozen = DiagGaussianActor(obs_dim=obs_dim,
                                       action_dim=action_dim,
                                       hidden_dim=hidden_dim,
                                       hidden_depth=hidden_depth,
                                       log_std_bounds=[-10, 10]).to(device)
        self.actor_frozen.load_state_dict(self.actor.state_dict())
    ...
    def update_critic(self, obs, action, reward, next_obs, not_done, use_wandb, step):
        ...
        # next_action = self.env_specs['action_space'].sample()
        ...
        # target_V = self.critic(next_obs, next_action)
        # target_Q = reward + (not_done * self.discount * target_V)
        # target_Q = target_Q.detach()
        #
        # # get current Q estimates
    ...
    def update_actor_and_alpha(self, obs, use_wandb, step):
        dist = self.actor(obs)
        dist_frozen = self.actor_frozen(obs)
        action = dist.rsample()
        ...
        true_actor_loss_temp = (- actor_Q)
        kl_loss_temp = torch.distributions.kl_divergence(dist_frozen, dist)
        kl_loss_temp = self.relu(kl_loss_temp - 0.0001)
        kl_loss_mean = kl_loss_temp.mean().detach().cpu().item()
        if kl_loss_mean < 0.01:
            kl_weight = self.kl_weight
        else:
            kl_weight = self.kl_weight * ((kl_loss_mean*300)**2)
        actor_loss = (kl_weight * kl_loss_temp + true_actor_loss_temp).mean()
        if use_wandb and step % 1000 == 0:
            wandb.log({'iter':step,
                       'actor train loss': actor_loss.detach().cpu().item(),
                       # 'actor entropy':-log_prob.mean().detach().cpu().item(),
                       'actor Q': (-true_actor_loss_temp).mean().detach().cpu().item(),
    ...
    def update(self, replay_buffer, use_wandb, step, step_per_inter=1):
        ...
        # if self.gather_actor_data < 10:
        #     self.gather_actor_data += 1
        # else:
        ...
        if (step % self.actor_update_frequency == 0) and (step > 5000):
            self.actor_frozen.load_state_dict(self.actor.state_dict())

        if step % self.critic_target_update_frequency == 0:
            utils.soft_update_params(self.critic, self.critic_target,
                                     self.critic_tau)
                    # self.gather_actor_data = 0
    #
