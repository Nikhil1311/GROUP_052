...
def train_agent(agent,
    ...
    while step < total_timesteps:
        ...
        if step % evaluation_freq == 0:
            ...
            if (avg_reward > current_best) and args.save_model:
        ...
        if (args.reset_every > 0) and (step % args.reset_every == 0):
            agent = Agent(env_specs=env_specs, device=args.device, hidden_dim=args.hidden_dim,
                          batch_size=args.batch_size, hidden_depth=args.hidden_depth,
                          actor_lr=args.learning_rate, critic_lr=args.learning_rate)
            # agent.reinit_model()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--evaluation_freq", type=int, default=500)
    ...
    if args.use_wandb:
        group_vars = ['total_timesteps', 'seed', 'n_episodes_to_evaluate', 'num_seed_steps', 'hidden_dim', 'hidden_depth',
                      'reset_every', 'step_per_inter', 'batch_size', 'learning_rate']

        group_name = ''
        for var in group_vars:
            group_name = group_name + '_' + var + str(getattr(args, var))
        wandb.init(project="comp579_sac_sample",
    ...
    agent = Agent(env_specs=env_specs, device=args.device, hidden_dim=args.hidden_dim, batch_size=args.batch_size,
                  hidden_depth=args.hidden_depth, actor_lr=args.learning_rate, critic_lr=args.learning_rate)
